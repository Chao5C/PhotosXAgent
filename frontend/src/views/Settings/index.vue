<template>
  <div class="settings-page">
    ...
    <el-tabs v-model="tab">
        ...
        <el-table :data="providers" v-loading="loading.providers" style="width: 100%">
          ...
          <el-table-column label="操作" width="360" fixed="right">
            <template #default="{ row }">
              ...
              <el-button v-if="row.name === 'ollama'" size="small" type="success" :loading="syncingOllama" @click="syncOllama">同步本机</el-button>
      ...
      <el-tab-pane label="大模型配置" name="models">
        ...
        <div v-for="group in modelGroups" :key="group.provider" class="provider-group">
          <div class="provider-header">
            ...
            <div>
              <el-button v-if="group.provider === 'ollama'" size="small" type="success" :loading="syncingOllama" @click="syncOllama">同步本机模型</el-button>
      ...
      <el-tab-pane label="MCP 网关" name="mcp">
        <el-alert type="info" :closable="false" show-icon style="margin-bottom: 16px">
          管理 PhotosXAgent 内置 MCP 服务（联网搜索、天气）。启用后，旅行攻略优先经 MCP 网关调用；可在下方测试连接。
        </el-alert>
        <div class="toolbar">
          <h3>MCP 网关</h3>
          <div class="toolbar-actions">
            <span class="muted">总开关</span>
            <el-switch v-model="mcpGateway.enabled" active-text="启用" inactive-text="禁用" @change="saveMcpGateway" />
            <el-button type="primary" :loading="savingMcp" @click="saveMcpGateway">保存配置</el-button>
          </div>
        </div>
        <el-table :data="mcpGateway.servers" v-loading="loading.mcp" border>
          ...
          <el-table-column label="启动命令" min-width="260">
            <template #default="{ row }">
              <div class="mono">{{ row.command }} {{ (row.args || []).join(' ') }}</div>
              <div class="muted">cwd: {{ row.cwd || '-' }}</div>
            </template>
          </el-table-column>
          <el-table-column label="状态" width="100" align="center">
            <template #default="{ row }">
              <el-switch v-model="row.enabled" @change="saveMcpGateway" />
            </template>
          </el-table-column>
          <el-table-column label="最近测试" min-width="220">
            <template #default="{ row }">
              <el-tag v-if="mcpTests[row.id]?.ok" type="success" size="small">成功</el-tag>
              <el-tag v-else-if="mcpTests[row.id]?.ok === false" type="danger" size="small">失败</el-tag>
              <span v-if="mcpTests[row.id]?.message" class="muted test-msg">{{ mcpTests[row.id]?.message }}</span>
              <span v-else class="muted">尚未测试</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="{ row }">
              <el-button size="small" type="primary" :loading="testingMcp === row.id" @click="testMcp(row.id)">测试连接</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
...
import { mcpApi, type McpGatewayConfig, type McpTestResult } from '@/api/mcp'
...
const testing = ref('')
const syncingOllama = ref(false)
const loading = reactive({ providers: false, catalogs: false, models: false, mcp: false })
...
const mcpGateway = reactive<McpGatewayConfig>({ enabled: true, servers: [] })
const mcpTests = reactive<Record<string, McpTestResult>>({})
...
const loadMcpGateway = async () => {
  loading.mcp = true
  try {
    const res = await mcpApi.gateway()
    mcpGateway.enabled = res.data?.enabled !== false
    mcpGateway.servers = res.data?.servers || []
  } finally {
    loading.mcp = false
  }
}
...
const saveMcpGateway = async () => {
  ...
  try {
    const res = await mcpApi.saveGateway({
      enabled: mcpGateway.enabled,
      servers: mcpGateway.servers || []
    })
    mcpGateway.enabled = res.data?.enabled !== false
    mcpGateway.servers = res.data?.servers || []
    ElMessage.success('MCP 网关配置已保存')
  } finally {
    savingMcp.value = false
  }
}

const testMcp = async (serverId: string) => {
  testingMcp.value = serverId
  try {
    const res = await mcpApi.test(serverId)
    mcpTests[serverId] = res.data || {}
    if (res.data?.ok) ElMessage.success(res.data.message || '连接成功')
...
const testProvider = async (row: any) => {
  ...
  try {
    ...
    const reply = res.data?.reply ? `：${res.data.reply}` : ''
    const endpoint = res.data?.base_url ? `（${res.data.base_url}）` : ''
    ElMessage.success((res.message || '连接成功') + endpoint + reply)
  } finally {
    testing.value = ''
  }
}
...
const syncOllama = async () => {
  syncingOllama.value = true
  try {
    const res = await api.syncOllama()
    ElMessage.success(res.message || `已同步 ${res.data?.count || 0} 个本机模型`)
    await loadAll()
  } finally {
    syncingOllama.value = false
  }
}
...
const testModel = async (row: any) => {
  ...
  try {
    ...
    const reply = res.data?.reply ? `：${res.data.reply}` : ''
    ElMessage.success((res.message || '模型可用') + reply)
  } finally {
    testing.value = ''
  }
}
